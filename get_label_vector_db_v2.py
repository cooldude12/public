...
def get_labels(data, is_DB=False, pipeline_job_id=0, question_id=0):
    ...
    if saved_version_path is None:
        print("no version is saved")
    else:
        print("saved_version_path " + saved_version_path)
    ...
    if saved_version_path and os.listdir(saved_version_path) and ST_SAVE:
        print("Files exisitng in the ST version folder, so loading the saved version")
        model = SentenceTransformer(saved_version_path)
    else:
        print("new question, no saved ST, so starting with vanilla ST")
        model = SentenceTransformer(model_name)
    ...
    print_debug("search results all: count: " + str(search_count_limit))
    print_debug(str(search_result))
    print_debug("***********************")
    ...
    for new_answer, result in zip(data, search_result):
        ...
        # Sort the labels based on distance (in ascending order)
        ...
        unique_labels = list(set(label for label, _ in sorted_labels))
        # Call filter_similar_labels with the unique labels
        ...
        sorted_filtered_labels = sorted(unique_labels_filtered, key=lambda x: label_score_dict[x])
        # Get the top k labels with the least distance
        ...
        output_records = [similar_question for similar_question in similar_questions if similar_question['label'] in unique_labels_filtered]
        ...
        # Create a subset of output_records containing only the top labels from unique_labels
        output_details_top = []
        for label in top_k_labels: #unique_labels:
        ...
        print_debug("unique labels " + str(unique_labels_str))
        print_debug("top labels " + str(output_details_top))
        ...
        new_answer_with_labels = [new_answer_id, new_answer_text, unique_labels, unique_labels_str]
...
def get_labels_old(data, is_DB=False, pipeline_job_id=0, question_id=0):
    # Initialize the SentenceTransformer model
    model = SentenceTransformer(model_name)
    collection_name = collection_name_label + '_' + str(question_id)
    print_debug(data)
